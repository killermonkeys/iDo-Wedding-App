module GuestsHelper
  ...
  def guest_status_content(guest, html_tag = :p, html_options = {})
    ...
    if guest.has_rsvped?
      ...
      html << 'If not, go ahead and click on the &ldquo;Update My Info&rdquo; button below.'
    end
  ...
  def guest_email_address_content(guest, html_tag = :p, html_options = {})
    ...
    if guest.email?
      ...
      html << "No email address entered, please add one: #{link_to 'Enter Email Address', @admin_user ? edit_admin_guest_path(guest) : edit_guest_path(:anchor => 'email'), :class => 'button_link' }"
    end    
    content_tag(html_tag, html.html_safe, html_options)
  end
end
